...
def write_raw_string(vi, wr_str):
    ...
    try:
        ...
        if err_code < 0:
            ...
            wrn_msg = 'write_raw_string(wr_str="{0}")={1} ({2})'
            wrn_msg = wrn_msg.format(wr_str, err_code, err_desc)
        ...
        wrn_msg = wrn_msg.format(wr_str, sys.exc_info())
        warnings.warn(wrn_msg)
...
def write_raw_bin_dat(vi, bin_dat, dat_size, max_chunk_size=1024):
    ...
    try:
        if hasattr(vi.visalib, 'viWrite'):
            ...
            if dat_size <= max_chunk_size:
                ...
                while wr_offs < dat_size:
                    ...
                    ptr = ctypes.cast(
                        ctypes.addressof(p_dat.contents) +
                        wr_offs, ctypes.POINTER(ctypes.c_byte))
                    err_code = vi.visalib.viWrite(
                        vi.session, ptr, ul_sz, p_cnt)
            ...
            if not isinstance(bin_dat, np.ndarray):
                bin_dat = np.ndarray(bin_dat)

            if dat_size <= max_chunk_size:
                ret_count, err_code = \
                    vi.write_raw(bin_dat.tobytes())
            else:
                bin_dat = bin_dat.tobytes()
                while wr_offs < dat_size:
                    chunk_sz = min(max_chunk_size, dat_size - wr_offs)
                    count, err_code = vi.write_raw(
                        bin_dat[wr_offs: wr_offs + chunk_sz])
                    if 0 != err_code:
                        break
                    elif count != chunk_sz:
                        ret_count = min(count, -1)
        ...
        if count < 0 or err_code < 0:
            err_desc = get_visa_err_desc(err_code)
            wrn_msg = \
                'write_raw_bin_dat(dat_size={0})={1}, ' + \
                'wr_offs={2}, err_code={3} ({4})'
            wrn_msg = wrn_msg.format(
                dat_size, count, wr_offs, err_code, err_desc)
    except visa.VisaIOError:
        ret_count = min(ret_count, -1)
        wrn_msg = 'write_raw_bin_dat(dat_size={0}) failed\n{1}'
        wrn_msg = wrn_msg.format(dat_size, sys.exc_info())
        warnings.warn(wrn_msg)
...
def try_read(vi, mstimeout=None):
    '''Try read SCPI response from the remote instrument.

    :param vi: `pyvisa` instrument.
    :param mstimeout: waiting timeout in milliseconds (optional).
    :returns: the response-string and boolean result (True if succeeded).
    '''
    resp = ''
    res = False
    ...
    if vi is not None:
        try:
            if mstimeout is not None:
                vi.timeout = mstimeout
...
def _pre_download_binary_data(vi, bin_dat_size=None):
    ...
    :returns: the max write-chunk size (in bytes)
              and the original time-out (in msec).
    '''
    orig_timeout = vi.timeout
    max_chunk_size = 4096
    if vi is not None:
...
def download_binary_data(vi, pref, bin_dat, dat_size, paranoia_level=1):
    ...
    Example:
        ...
        >>> _ = vi.query('*RST; *CLS; *OPC?') # reset the instrument
        >>> _ = vi.query(':FUNC:MODE USER; *OPC?') # select arbirary-wave mode
        >>> _ = vi.query(':FREQ::RAST 2GHz; *OPC?') # Set sampling-rate = 2GHz
        >>>
        ...
        >>>
        >>> _ = vi.query(':OUTP ON; *OPC?') # turn on the active channel
        >>> print vi.query(':SYST:ERR?')
        >>> vi.close()
    """
    ret_count = 0
    ...
    try:
        ...
        try:
            ...
            if ret_count < 0:
                ...
                count = write_raw_bin_dat(
                    vi, bin_dat, dat_size, max_chunk_size)
                if count < 0:
                    ret_count = count
        ...
        if paranoia_level >= 2:
            syst_err = vi.query(':SYST:ERR?')
            if not syst_err.startswith('0'):
                syst_err = syst_err.rstrip()
                wrn_msg = \
                    'ERR: "{0}" after sending binary data ' + \
                    '(pref="{1}", dat_size={2})'
        ...
        if ret_count >= 0:
            ret_count = -1
        wrn_msg = \
            'Error in download_binary_data(' + \
            'pref="{0}", dat_size={1}): \n{2}'
        wrn_msg = wrn_msg.format(pref, dat_size, sys.exc_info())
        if paranoia_level >= 3:
            raise NameError(wrn_msg)
        else:
            warnings.warn(wrn_msg)
...
def download_binary_file(
    ...
    Example:
        ...
        >>> _ = vi.query(':FREQ::RAST 2GHz; *OPC?') # Set sampling-rate = 2GHz
        >>>
        >>> # write wave-data from file to the
        >>> # active segment of the active channel:
    ...
    with open(file_path, mode='rb') as infile:
        try:
            ...
            if data_size > 0:
                orig_timeout, max_chunk_size = \
                    _pre_download_binary_data(vi, data_size)
                ...
                try:
                    ...
                    if ret_count < 0:
                        ...
                        while offset < data_size and ret_count >= 0:
                            ...
                            chunk = np.fromfile(
                                infile, dtype=np.uint8, count=chunk_size)
                            count = write_raw_bin_dat(
                                vi, chunk, chunk_size, max_chunk_size)
                            if count < 0:
                                ret_count = count
                                wrn_msg = "Failed to write binary-data "
                                warnings.warn(wrn_msg)
                ...
                if paranoia_level >= 2:
                    ...
                    if not syst_err.startswith('0'):
                        ...
                        wrn_msg = \
                            'ERR: "{0}" after sending binary data ' + \
                            '(pref="{1}", dat_size={2})'
                        wrn_msg = wrn_msg.format(syst_err, pref, data_size)
                        warnings.warn(wrn_msg)
                        _ = vi.query('*CLS; *OPC?')  # clear the error-list
        except visa.VisaIOError:
            if ret_count >= 0:
            ...
            wrn_msg = \
                'Error in download_binary_data' +\
                '(pref="{0}", data_size={1}): \n{2}'
            wrn_msg = wrn_msg.format(pref, data_size, sys.exc_info()[0])
            warnings.warn(wrn_msg)

    return ret_count


def download_arbcon_wav_file(
        vi,
        file_path, pref=':TRAC:DATA',
        bits_per_point=14,
        paranoia_level=1):
    """Download wave data from binary wav file created by the ArbConnection.
    ...
    with open(file_path, mode='rb') as infile:
        try:
            ...
            if wav_len > 0:
                data_size = wav_len * 2
                orig_timeout, max_chunk_size = \
                    _pre_download_binary_data(vi, wav_len * 2)
                ...
                try:
                    ...
                    if ret_count < 0:
                        ...
                        while offset < wav_len and ret_count >= 0:
                            chunk_size = \
                                min(wav_len - offset, max_chunk_size // 2)
                            chunk = np.fromfile(
                                infile, dtype=np.uint16, count=chunk_size) \
                                + 2**(bits_per_point-1)
                            chunk = np.clip(chunk, 0, 2**bits_per_point-1)
                            count = write_raw_bin_dat(
                                vi, chunk, chunk.nbytes, chunk.nbytes)
                            if count < 0:
                                ret_count = count
                                wrn_msg = "Failed to write binary-data "
                                warnings.warn(wrn_msg)
...
def download_sequencer_table(
        vi, seq_table, pref=':SEQ:DATA', paranoia_level=1):
...
def download_adv_seq_table(
        vi, adv_seq_table, pref=':ASEQ:DATA', paranoia_level=1):
...
def download_fast_pattern_table(
        vi, patt_table, pref=':PATT:COMP:FAST:DATA', paranoia_level=1):
    ...
    Example:
        ...
        >>> # - Pattern-Mode=Composer,
        >>> # - Pattern-Type=Fast.
        >>> _ = vi.query(
        >>>    ':FUNC:MODE PATT; :PATT:MODE COMP; ' +
        >>>    ':PATT:COMP:TRAN:TYPE FAST; *OPC?')
        >>>
        >>> # Optionally set User-Defined point sampling time
        >>> # (rather than Auto point sampling time)
        >>> _ = vi.query(
    ...
    return download_binary_data(
        vi, pref, m, m.nbytes, paranoia_level=paranoia_level)
...
def download_linear_pattern_table(
    ...
    if start_level is not None:
        ...
        _ = vi.query(':PATT:COMP:LIN:STARt {0:f}; *OPC?'.format(start_level))
...
def build_sine_wave(
    ...
    x = np.linspace(
        start=phase, stop=phase+2*math.pi, num=cycle_len, endpoint=False)
...
def add_markers(
    ...
    IMPORTANT: This function currently fits only 4-channels devices
               (WX2184 / WX1284).

    :param dat_buff: `numpy` array of 'uint16' containing the wave-data.
    :param marker_pos: the marker start-position (in wave-points)
    ...
    :param dat_offs: the offset of the wave-data within the data-buffer.
    :param dat_len: the length of the actual wave-data,
                    defaults to the length of `dat_buff`.
    """
...
def make_combined_wave(
    ...
    The destination-array, `dest_array`, is either a `numpy.array`
    with `dtype=uint16`, or `None`. If it is `None` then only the
    next destination-array's write-offset offset is calculated.

    Each of the given waves, `wav1` and `wav2`,
    is either a `numpy.array` with `dtype=uint16`, or `None`. If it
    is `None`, then the corresponding entries of `dest_array` are not changed.

    :param wav1: the DAC values of wave 1
                 (either `numpy.array` with `dtype=uint16`, or `None`).
    :param wav2: the DAC values of wave 2
                 (either `numpy.array` with `dtype=uint16`, or `None`).
    ...
    if min(quantum, len2) > 0:
        ...
        while rd_offs < len2 and wr_offs < dest_len:
            ...
            dest_array[wr_offs: wr_offs + chunk_len] = \
                wav2[rd_offs: rd_offs + chunk_len]
    ...
    if min(quantum, len1) > 0:
        ...
        while rd_offs < len1 and wr_offs < dest_len:
            ...
            dest_array[wr_offs: wr_offs + chunk_len] = \
                wav1[rd_offs: rd_offs + chunk_len]
            rd_offs = rd_offs + chunk_len
            wr_offs = wr_offs + chunk_len + quantum
